...
class GearmanConnection(object):
    """A connection between a client/worker and a src.  Can be used to reconnect (unlike a socket)
    ...
    def connect(self):
        """Connect to the src. Raise ConnectionError if connection fails."""
        if self.connected:
    ...
    def _unpack_command(self, given_buffer):
        """Conditionally unpack a binary command or a text based src command"""
    ...
    def _pack_command(self, cmd_type, cmd_args):
        ...
        if cmd_type == GEARMAN_COMMAND_TEXT_COMMAND:
            ...
            # We'll be sending a response if we know we're a src side command
