...
from .constants import PRIORITY_NONE, JOB_UNKNOWN, JOB_PENDING
...
class GearmanClient(GearmanConnectionManager):
    """
    GearmanClient :: Interface to submit jobs to a Gearman src
    """
    ...
    def submit_job(self, task, data, unique=None, priority=PRIORITY_NONE, **kwargs):
        """Submit a single job to any gearman src"""
        job_info = {
            "task": task,
            "data": data,
            "unique": unique,
    ...
    def establish_request_connection(self, current_request):
        ...
        for possible_connection in rotating_connections:
            try:
                ...
                # Rotate our src list so we'll skip all our broken servers
