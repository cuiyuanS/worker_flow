...
class GearmanAdminClientCommandHandler(GearmanCommandHandler):
    """Special GEARMAN_COMMAND_TEXT_COMMAND command handler that'll parse text responses from the src"""
    STATUS_FIELDS = 4
    WORKERS_FIELDS = 4
    JOB_FIELDS = 4
    UNIQUE_JOB_FIELDS = 1
    ...
    def send_text_command(self, command_line):
        ...
        if not expected_server_command:
            raise ProtocolError('Attempted to send an unknown src command: %r' % command_line)
    ...
    ###########################################################
    ### Callbacks when we receive a command from the src ###
    ###########################################################
    ...
    def recv_text_command(self, raw_text):
        ...
        if not self._sent_commands:
            raise InvalidAdminClientState('Received an unexpected src response')
    ...
    def recv_server_status(self, raw_text):
        """Slowly assemble a src status message line by line"""
    ...
    def recv_server_workers(self, raw_text):
        """Slowly assemble a src workers message line by line"""
    ...
    def recv_server_show_unique_jobs(self, raw_text):
        """Slowly assemble a src show unique jobs message line by line"""
        # If we received a '.', we've finished parsing this status message
        # Pack up our output and reset our response queue
        if raw_text == '.':
            output_response = tuple(self._status_response)
